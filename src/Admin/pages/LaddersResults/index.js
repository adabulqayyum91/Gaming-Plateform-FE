...
export default function LaddersResults() {
  ...
  const { laddersResults, exporturl } = useSelector(
    (state) => state.laddersResults
  );
  const { data, total, page } = laddersResults;
  const { TblContainer, TblHead, TblToolbar, ComponentContainer, Pagination } =
    useTable(data, headCells);
  let pageCpy = page;

  useEffect(() => {
    dispatch(getLaddersResults({ query: "" }));
  }, []);
  useEffect(() => {
  ...
  const handleChangePage = (event, newPage) => {
    ...
    dispatch(getLaddersResults({ query: "", pageNo: ++newPageCpy }));
  };
  ...
  const confirmDeleteHandler = () => {
    ...
    dispatch(deleteLaddersResult({ resultId: delObj }));
    setDeleteModal(false);
    setSelected([]);
  };
  const searchHandler = (val) => {
    setSearchedVal(val);
    dispatch(getLaddersResults({ query: val }));
  };
                ...
                data.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        {row.gameToPlay.gameName}
                      </TableCell>
                      <TableCell align="center">
                        {tooltipTrim(row?.ladderId?.ladderName, 20)}
                      </TableCell>
